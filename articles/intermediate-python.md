...
しかし、pyenv はバージョンを切り替えることはできますが、仮想環境を作成できません。
この仮想環境とはプロジェクトごとに利用する環境のことを本記事では指します。

もし仮想環境を切り分けられない場合を考えてみます。
機械学習のプロジェクトを開発した後、別のプロジェクトに割り当てられ Django の開発になったとします。
...
flake8 は、一行の文字数など、細かい規約を設定できます。
...
しかし、利用しているライブラリなどに対してエラーを出すこともあります。
このような場合は、スタブを生成したり、すでに pip で配布されているスタブをインストールする必要があります。
...
`setup.py`はプロジェクトを第三者に向けて配布するために利用するファイルです。
...
普段何気なく書いている `pip install numpy` というコマンドは、setup.py で作成されたパッケージをオンライン（PyPI）から取得します。
そして、`site-packages`というディレクトリに入れています。
...
これまでは、`requirements.txt`, `setup.py`, `setup.cfg`, `MANIFEST.in`など多くのファイルがパッケージ公開に必要でした。
しかし、 pyproject.toml はこのファイルのみでこれらをすべて補う機能を果たします。
...
doctest をインポートすると、もし docstring に `>>>` という実行例が書いてあったら、その通りの動作になるかをテストしてくれます。
...
さきほど出てきましたが、tox を書くことで複数のテスト・コマンドを自動化できます。
...
この Sphinx には、自動でドキュメントを作成する機能である `sphinx-apidoc` が付属しておりいます。
そのため、docstring を Python コードにきちんと書いていれば、コマンド一発でドキュメントを作成できます。
